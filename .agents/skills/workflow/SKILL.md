...
派生规则：写操作（开发/修复）一次只派一个子代理以避免写入冲突；**提交前的只读审查阶段（安全核查与代码审查）由主 agent 视风险自主决定串行或并行**：
- **并行模式（默认）**：常规特性、日常修复、单测补充等，暂存后双工并行扇出（Fan-out）派发以最大化流转效率。
- **串行模式（高风险优先）**：涉及鉴权认证模块、密钥与凭据管理、.env 机制重构等存在高泄漏风险的任务，优先串行执行安全核查，发现严重泄漏立即熔断，确认安全后再派发代码审查。
子代理看不到主对话，**brief 必须自包含**。
...
4. **审查阶段（提交前）**：暂存代码（`git add`）后，主 agent 评估风险自主决定执行模式：
   - **并行（默认）**：同时派发 security-auditor 与 reviewer；
   - **串行（高敏感）**：先派 security-auditor，通过后再派 reviewer；
   - gitleaks hook 强制暂存扫描。
5. **修复**：任一审查提出 blocking 即返回 implementer 修复（安全泄漏阻断优先级最高）；advisory/nit 由主 agent 权衡。
6. **合回**：两项审查均 PASS 且测试通过后合回 `develop`。
7. **里程碑验收**：派 e2e-test-writer。
8. **推送**：向用户确认；推送前再过一遍安全核查（全仓）。
