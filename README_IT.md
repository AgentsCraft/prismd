...
3. **Fallback Locale Opzionale (Ollama / LM Studio)**: Gli alias predefiniti sono solo cloud. Hai un backend locale? Aggiungilo alla coda di un alias tramite `config.user.json` — quando i modelli cloud esauriscono i limiti o la rete cade, le richieste passano ai tuoi modelli locali.
...
2. Selezionare i provider gratuiti (OpenRouter, Groq, Google Gemini, ecc.) e inserire le chiavi API.
3. **Configurare automaticamente i tuoi client di coding** (Claude Code, Codex CLI, OpenCode, Pi Agent) con backup automatici (`.bak.<timestamp>`)!

*(Preferisci la configurazione manuale? Modifica `~/.prismd/keys.yaml` o imposta `PRISMD_HOME`).*
...
| Client | Avvio Rapido (`prismd init` auto-config) | Guida |
|---|---|---|
| **Claude Code** | `claude` (configurato in `~/.claude/settings.json`) | [Guida](examples/claude-code/README.md) |
