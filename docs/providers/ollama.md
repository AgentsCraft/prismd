...
prismd 原生内置 `ollama` 提供商支持（默认端点 `http://127.0.0.1:11434/v1`，无需 API Key 鉴权）。当云端免费 API 全部触发 429 限流或网络中断时，网关会自动无缝故障转移至本地 Ollama 实例，保障编码任务不中断。
...
- **自动兜底优先级**：在内置别名（如 `free-auto`）中，`qwen2.5-coder:7b` 默认位于候选列表末尾。仅当排在前面的云端免费服务全部降权或冷却时，请求才会分发至本地 Ollama。

---

## 3. 自定义 Ollama 地址与模型 (`config.user.json`)
